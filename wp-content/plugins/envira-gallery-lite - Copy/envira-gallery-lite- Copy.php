...
class Envira_Gallery_Lite {
	 ...
	 * @var string
	 */
	public $plugin_slug = 'envira-gallery-lite - Copy';
	...
	public function load_plugin_textdomain() {
		...
		load_plugin_textdomain( 'envira-gallery-lite - Copy' );
...
function envira_gallery_lite_activation_hook( $network_wide ) {
	...
	if ( version_compare( $wp_version, '5.0', '<' ) && ! defined( 'ENVIRA_FORCE_ACTIVATION' ) ) {
		...
		/* translators: %s: url */
		wp_die( sprintf( wp_kses_post( __( 'Sorry, but your version of WordPress does not meet Envira Gallery\'s required version of <strong>5.0</strong> to run properly. The plugin has been deactivated. <a href="%s">Click here to return to the Dashboard</a>.', 'envira-gallery-lite - Copy' ), esc_url( get_admin_url() ) ) ) );
	}
