...
class Envira_Gallery_Addons {
	...
	public function admin_menu() {
		...
		$this->hook = add_submenu_page(
			...
			__( 'Envira Gallery Addons', 'envira-gallery-lite - Copy' ),
			'<span style="color:#7cc048"> ' . __( 'Addons', 'envira-gallery-lite - Copy' ) . '</span>',
	...
	public function addons_page() {
		...
		<div id="addon-heading" class="subheading clearfix">
			<h1><?php esc_html_e( 'Envira Gallery Addons', 'envira-gallery-lite - Copy' ); ?></h1>
			<form id="add-on-search">
				<span class="spinner"></span>
				<input id="add-on-searchbox" name="envira-addon-search" value="" placeholder="<?php esc_attr_e( 'Search Envira Addons', 'envira-gallery-lite - Copy' ); ?>" />
				<select id="envira-filter-select">
					<option value="sort-order"><?php esc_html_e( 'Most Popular', 'envira-gallery-lite - Copy' ); ?></option>
					<option value="asc"><?php esc_html_e( 'Sort Ascending (A-Z)', 'envira-gallery-lite - Copy' ); ?></option>
					<option value="desc"><?php esc_html_e( 'Sort Descending (Z-A)', 'envira-gallery-lite - Copy' ); ?></option>
				</select>
			</form>
		</div>
	...
	public function addons_content() {
		...
		if ( false !== $this->base->get_license_key_errors() ) {
			...
			<div class="error below-h2">
				<p>
					<?php esc_html_e( 'In order to get access to Addons, you need to resolve your license key errors.', 'envira-gallery-lite - Copy' ); ?>
				</p>
			</div>
			<?php
			return;
		...
		if ( ! $addons ) {
			...
			<form id="envira-addons-refresh-addons-form" method="post">
				<p>
					<?php esc_html_e( 'There was an issue retrieving the addons for this site. Please click on the button below the refresh the addons data.', 'envira-gallery-lite - Copy' ); ?>
				</p>
				<p>
					<?php if ( ! empty( $_SERVER['REQUEST_URI'] ) ) { ?>
					<a href="<?php echo esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Refresh Addons', 'envira-gallery-lite - Copy' ); ?></a>
					<?php } ?>
				</p>
			</form>
			<?php
		...
		<div id="envira-addons">
			...
			if ( count( $addons['licensed'] ) > 0 ) {
				...
				<div id="envira-addons-area-license" class="envira-addons-area licensed" class="envira-clear">
					<h3><?php esc_html_e( 'Available Addons', 'envira-gallery-lite - Copy' ); ?></h3>
			...
			if ( count( $addons['unlicensed'] ) > 0 ) {
				...
				<div id="envira-addons-area-unlicensed" class="envira-addons-area unlicensed" class="envira-clear">
					<h3><?php esc_html_e( 'Unlock More Addons', 'envira-gallery-lite - Copy' ); ?></h3>
					<?php /* translators: %s: url */ ?>
					<p><?php echo sprintf( wp_kses_post( __( '<strong>Want even more addons?</strong> <a href="%s">Upgrade your Envira Gallery account</a> and unlock the following addons.', 'envira-gallery-lite - Copy' ) ), esc_url( $upgrade_url ) ); ?></p>
		...
		$addons_link = sprintf(
			...
			esc_url(
				add_query_arg(
					array(
						'post_type' => 'envira',
						'page'      => 'envira-gallery-lite - Copy-addons',
					),
					admin_url( 'edit.php' )
				)
			),
			__( 'Addons', 'envira-gallery-lite - Copy' )
		);
			...
			if ( ! $is_licensed ) {
					...
					<div class="interior">
						<div class="envira-addon-upgrade">
							<a href="<?php echo esc_url( $addon->upgrade_url ); ?>" target="_blank" class="button button-primary envira-addon-upgrade-button"  rel="<?php echo esc_attr( $plugin_basename ); ?>">
								<?php esc_html_e( 'Upgrade Now', 'envira-gallery-lite - Copy' ); ?>
							</a>
				...
				if ( ! isset( $installed_plugins[ $plugin_basename ] ) ) {
					...
					<div class="envira-addon-not-installed envira-addon-message">
						<div class="interior">
							<span class="addon-status"><?php esc_html_e( 'Status:', 'envira-gallery-lite - Copy' ); ?>&nbsp;<span><?php esc_html_e( 'Not Installed', 'envira-gallery-lite - Copy' ); ?></span></span>
							<div class="envira-addon-action">
								<a class="button button-primary envira-addon-action-button envira-install-addon" href="#" rel="<?php echo esc_url( $addon->url ); ?>">
									...
									<?php esc_html_e( 'Install', 'envira-gallery-lite - Copy' ); ?>
								</a>
								<span class="spinner envira-gallery-spinner"></span>
							</div>
						</div>
					...
					if ( is_plugin_active( $plugin_basename ) ) {
						...
						<div class="envira-addon-active envira-addon-message">
							<div class="interior">
								<span class="addon-status"><?php esc_html_e( 'Status:', 'envira-gallery-lite - Copy' ); ?>&nbsp;<span><?php esc_html_e( 'Active', 'envira-gallery-lite - Copy' ); ?></span></span>
								<div class="envira-addon-action">
									<a class="button button-primary envira-addon-action-button envira-deactivate-addon" href="#" rel="<?php echo esc_attr( $plugin_basename ); ?>">
										...
										<?php esc_html_e( 'Deactivate', 'envira-gallery-lite - Copy' ); ?>
									</a>
									<span class="spinner envira-gallery-spinner"></span>
								</div>
							</div>
						...
						<div class="envira-addon-inactive envira-addon-message">
							<div class="interior">
								<span class="addon-status"><?php esc_html_e( 'Status:', 'envira-gallery-lite - Copy' ); ?>&nbsp;<span><?php esc_html_e( 'Inactive', 'envira-gallery-lite - Copy' ); ?></span></span>
								<div class="envira-addon-action">
									<a class="button button-primary envira-addon-action-button envira-activate-addon" href="#" rel="<?php echo esc_attr( $plugin_basename ); ?>">
										<i class="envira-toggle-on"></i>
										<?php esc_html_e( 'Activate', 'envira-gallery-lite - Copy' ); ?>
									</a>
									<span class="spinner envira-gallery-spinner"></span>
								</div>
							</div>
