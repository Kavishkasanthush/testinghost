...
<tr>
	...
	public function payhere_log( $type, $data ) {
		...
		if ( is_array( $data ) || is_object( $data ) ) {
			$__content = wp_json_encode( $data );
		}
		$_content = gmdate( 'Y-m-d H:i:s' ) . "| [$type] \t\t| $__content";

		// Reason : Only logs events to a file. No need of complex code.
		// phpcs:ignore
		file_put_contents( $logs_dir . '/log_' . gmdate( 'Y-m-d' ) . '.log', array( $_content . '' . PHP_EOL ), FILE_APPEND );
	}
}
